...
\textbf{Time Complexity:} $O(V + E)$\\ \textbf{Space Complexity:} $O(V)$\\[-17px]
\algocode{bfs}
\end{minipage}
\begin{minipage}[t]{0.45\linewidth}
    \vspace*{-30px}
...
\textbf{DFS-Visit Time Complexity:} $O(E)$\\
\textbf{DFS-Visit Space Complexity:} $O(V)$\\
\textbf{Time Complexity:} $O(V + E)$\\ \textbf{Space Complexity:} $O(V)$\\[-13px]
